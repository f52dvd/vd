...
var BatchDownload={
	...
	GEBcheckbox : "",
	Binding : function() {
		GEBcheckbox = document.getElementsByName('checkbox')
		for(i=0; i<k.length; i++) {
			if(k.item(i).checked==true){
...
function checkform(form){
	...
	if(window.location.pathname.indexOf("/search/")==-1)form.target="_blank";
	if (value.length<1){
		alert('请输入搜索关键词');
		form.keyword.focus();
		return false;
...
function DownloadByThunder(url,pid){
    ...
    if (npXLPlugin &&  xlPlugin.DownLoadByThunderPlugin) {
        ...
        alert("请使用IE下载，360浏览器请开启兼容模式或执行以下操作\r\n\r\n"+thunderLanguage["Thunder5SetUpInfo"]);
        setTimeout(function(){top.location.href= wtd_ChangFolder(pid);},200);
    }
}
function iPhoneDownload(sDownloadURL, sPid)
{
    if(!/ithunder/i.test(navigator.userAgent.toLowerCase()))
    {
        setTimeout(openAppstore, 1000);
    }
	else
	{
...
function openAppstore() {
	window.location= 'https://itunes.apple.com/us/app/xun-lei/id593444693?ls=1&mt=8';
}
...
{
	if(navigator.plugins[i].filename == "http://www.poxiao.com/template/default/images/npXBrowserAgent.dll")
	{
	 var  fFpluginXB=true;
	}
}
...
function wtd_ChangFolder(pid)
{
 ...
 if(thunder_Contents != null && thunder_Contents > 0)
 {
   ...
   {
     return "http://down.sandai.net/thunder7/Thunder_dl_7.9.22.4780.exe";
   }
   else
   {
	   return "http://down.sandai.net/thunder7/Thunder_dl_7.9.22.4780.exe";
...
var linkObj;
var sfpl=0;
function OnDownloadClick_Simple(linkObj)
{
    ...
    if(sDownload == ""){
     ...
     thunder_linkType=1;
	var isOpenNewWindow=2,selectType=4;
	...
	if(thunder_isIE)
	{
		...
		Thunder.download(null,null,null,null);
		
		Thunder.thunderType		=oldTType;
		Thunder.isOpenNew		=oldTOpen;
		Thunder.mustUseSelected	=oldMustUse;
		if(sfpl==0)Thunder.client.commitTasks();
	}
	else if(osType == 'iPhone' || osType == 'iPad' || osType=='iPod')
	{
        iPhoneDownload(sDownload,sPid);
	}
	else
	{
		if(!thunder_isIE){
...
function ThunderNetwork_SetHref(linkObj)
{
	...
	//var urlcs = linkObj.offsetParent.all[0].value.split("&");
	//furl=request(urlcs,"xzurl");
	//cid=request(urlcs,"cid");
	//if(furl){
	//	linkObj.href = furl;
	//	document.getElementById("thunder_cid").value = "";
	//	document.getElementById("thunder_down_pageurl").value = furl;
	//	document.getElementById("thunder_stat_pageurl").value = location.href;
	//}else{
	//	document.getElementById("thunder_cid").value = cid;
	//	document.getElementById("thunder_down_pageurl").value = "http://thunder.ffdy.cc/" + cid + "/" + linkObj.innerText;
	//	document.getElementById("thunder_stat_pageurl").value = location.href;
	//	linkObj.href = "http://thunder.ffdy.cc/" + cid + "/" + linkObj.innerText;
		//alert("此资源请勿使用右键进行下载！\n否则无法下载正确的文件");
		//return false;
	//}
}
...
thunderLanguage["Thunder5SetUpInfo"]	=unescape("%u6B64%u94FE%u63A5%u4E3A%u8FC5%u96F7%u4E13%u7528%u4E0B%u8F7D%u901A%u9053%uFF0C%u5FC5%u987B%u5B89%u88C5%u8FC5%u96F77%u624D%u80FD%u8FDB%u884C%u4E0B%u8F7D%uFF0C%u5B89%u88C5%u540E%u8BF7%u91CD%u65B0%u8FDB%u5165%u6B64%u9875%u8FDB%u884C%u4E0B%u8F7D%u3002");
...
var Thunder = {
	...
	download: function( url, refer, name, stat,cid){
		...
		if(!this.client){			
			if(!this.isIE && (this.client==0)){}else{this.showSetUpInfo("14103");}
		}else{					
			if (this.isIE)
			{			
	...
	showSetUpInfo:function(pid){
		...
		if(this.isOpenNew){
			...
			if(pid.substr(0,1)=="g")
				url="http://www.poxiao.com/xunlei-0301/";
			else
				url="http://www.poxiao.com/xunlei-0301/";		
		}else{
			if(this.isIE)
...
Thunder.Thunder5.prototype = {
	...
	addTask: function(){
        var urlcs = linkObj.offsetParent.all[0].value.split("&");
		var name=linkObj.innerText;
		var url=request(urlcs,"xzurl");
        var urlcs1 = url.split("&");
		var cid=request(urlcs1,"cid");
		var mc=request(urlcs,"mc");
	//alert(urlcs);
		var pmc;
		if (cid==""){
		url=request(urlcs,"xzurl");
		}else{
		url="http://thunder.ffdy.cc/" + cid + "/"+name;
		
		}
		name = name.replace(/\\|\/|:|\*|\?|"|<|>|\|/g," ");
		url=ThunderEncode(url);
		var _addTask = [
			Delegate.create(this, function(){this.__thunder.AddTask4(url, name, "", pmc, refer, -1, 0, -1, document.cookie, cid, "");}),
			Delegate.create(this, function(){this.__thunder.AddTask3(url, name, "", pmc, refer, -1, 0, -1, document.cookie, cid);}),
			Delegate.create(this, function(){this.__thunder.AddTask2(url, name, "", pmc, refer, -1, 0, -1, document.cookie);}),
			Delegate.create(this, function(){this.__thunder.AddTask(url, name, "", pmc, refer, -1, 0, -1);})
		];
...
Thunder.MiniThunder.prototype = {
	...
	addTask: function(url, refer, name, cid, c){
		var urlcs = linkObj.offsetParent.all[0].value.split("&");
		var url=request(urlcs,"xzurl");
		var cid=request(urlcs,"cid");
		var name=linkObj.innerText;
		var pmc;//="FFDY：" + request(urlcs,"mc");
		url="http://thunder.ffdy.cc/" + cid + "/" + name;
		if (cid=="")url=request(urlcs,"xzurl");
		url= url + "&name=" + encodeURI(name);
		if (linkObj.innerText.indexOf("[BT]")==0){
			url="http://torrent.ffdy.cc/"+cid.slice(0,2)+"/"+cid.slice(cid.length-2,cid.length)+"/"+cid+".torrent";
		}else if(linkObj.innerText.indexOf("[EMULE]")==0){
			alert("电驴下载制作中");
			return;
		}else{
			if (request(urlcs,"xzurl")!="") url=request(urlcs,"xzurl");
		}
		pmc = "FFDY：" + name;
		name = name.replace(/\\|\/|:|\*|\?|"|<|>|\|/g," ");
		url=ThunderEncode(url);
		this.__thunder.AddTask(url, refer, name, cid, document.cookie);
	},
...
function zhongxz() {
	if(thunder_isIE)
	{
		if (Thunder.getInstance()) {
			sfpl = 1;
			var jjjj=0;
			for (var i = 0; i < GEBcheckbox.length; i++) {
				if (GEBcheckbox.item(i).checked == true) {
					OnDownloadClick_Simple(GEBcheckbox.item(i).offsetParent.all[1]);
					jjjj+=1;
				}
			}
			if (jjjj==0)
	...
	else
	{
		...
		if (npXLPlugin2) 
		{
			...
			var TempJq=jQuery(".sebc2 a.pClass");
			// LinkCount（任务个数）
            strUrls = strUrls.concat(TempJq.length, g_strSplitter);
			for (var i = 0; i < TempJq.length; i++) {
				strUrls = strUrls.concat(TempJq.eq(i).attr("thunderHref"), g_strSplitter);
				// filename 当filename与url相同时，置为空
				if(TempJq.eq(i).attr("thunderHref") == TempJq.eq(i).attr("thunderResTitle"))
				{
					strUrls = strUrls.concat('', g_strSplitter);
				}
				else{
					strUrls = strUrls.concat(TempJq.eq(i).attr("thunderResTitle"), g_strSplitter);
				}
				// Comments（任务注释）
				strUrls = strUrls.concat("", g_strSplitter);
				// Cookie
		...
		{
			var r = confirm("请先下载迅雷安装或升级，是否下载迅雷？");
			if(r == true){
				window.location.href= wtd_ChangFolder(14103);	
			}
		}
	}
}
function xz(t){
	for(i=0; i<GEBcheckbox.length; i++)GEBcheckbox.item(i).checked=t;
	//zdx();
}
var sfkkbf;
function appInfo(){
...
var myos = appInfo();
function pkkisz() {
	var TempJq=jQuery(".gkfj ul li a");
    var GEBcheckbox = jQuery('.sebc2 input[name="checkbox"]');
    if (TempJq.length == GEBcheckbox.length) {
        for (var i = 0; i < TempJq.length; i++) {
			TempJq.eq(i).attr("href","#p"+(i+1));
			TempJq.eq(i).attr("onClick","kPlay("+i+")");
			TempJq.eq(i).removeAttr("target");
        }
    } else {
    }
} 
...
document.getElementsByName('qx').item(0).onclick = function(dd) {
	if (this.checked){
		xz(true);
		} else{
			xz(false);
...
var trd,tdd,urlcs,cid,url,ahref,furl;
var TempJq,TempJqList;
TempJqList=jQuery(".sebc2 a:not(.XL_CLOUD_VOD_BUTTONyundianbo_btn_normal)");
for(var i=0; i<GEBcheckbox.length; i++) {
trd=GEBcheckbox[i].parentNode.parentNode
urlcs = GEBcheckbox[i].value.split("&");
furl=request(urlcs,"xzurl");
var urlcs2 = furl.split("&");
cid=request(urlcs2,"cid");
//alert(cid);
tdd = document.createElement("td");
tdd.className=trd.cells[0].className;
TempJq=TempJqList.eq(i);
TempJq.addClass("pClass")


if(cid==""){
TempJq.attr("thunderHref",furl);
tdd.innerHTML='<a class="XL_CLOUD_VOD_BUTTONyundianbo_btn_normal" target="_blank" href="http://vod.lixian.xunlei.com/share.html?from=un_14103&url='+encodeURIComponent(furl)+'"></a> <a href=\"#\" onclick=\"return kPlay('+i+')\" class=\"xmp\">看看播放</a>';
}else{
TempJq.attr("thunderHref","http://thunder.ffdy.cc/" + cid + "/" + (GEBcheckbox[i].parentNode.textContext == undefined ? GEBcheckbox[i].parentNode.innerText: GEBcheckbox[i].parentNode.textContext) + "");
tdd.innerHTML='<a class="XL_CLOUD_VOD_BUTTONyundianbo_btn_normal" target="_blank" href="http://vod.lixian.xunlei.com/share.html?from=un_14103&url='+encodeURIComponent("http://thunder.ffdy.cc/"+cid+"/"+(GEBcheckbox[i].parentNode.textContext == undefined ? GEBcheckbox[i].parentNode.innerText : GEBcheckbox[i].parentNode.textContext)) +'"></a>';
tdd.innerHTML='<a class="XL_CLOUD_VOD_BUTTONyundianbo_btn_normal" target="_blank" href="http://vod.lixian.xunlei.com/share.html?from=un_14103&url='+encodeURIComponent(furl)+'"></a> <a href=\"#\" onclick=\"return kPlay('+i+')\" class=\"xmp\">看看播放</a>';
}
TempJq.attr("thunderResTitle",TempJq.html());
TempJq.attr("thunderPid","");
TempJq.attr("title",TempJq.html());
//kis=kkis();
//if(kis){
//	tdd.innerHTML="<a href=\"#\" onclick=\"return kPlay(this)\" class=\"xmp\">看看播放</a>";
//}else{
//	ahref="http://www.poxiao.com/template/default/images/lixian.vip.xunlei.com/lixian_login.html?referfrom=union&ucid=14103";
//	ahref +="&furl="+encodeURIComponent("http:thunder.ffdy.cc/"+cid+"/"+GEBcheckbox[i].parentNode.innerText);
//	if(furl){
//		ahref ="http://www.poxiao.com/template/default/images/lixian.vip.xunlei.com/lixian_login.html?referfrom=union&ucid=14103&furl="+encodeURIComponent(furl);
//	}else{
//		ahref +="&cid="+cid;
//	}
//		ahref +="&s=FFDY：" + encodeURIComponent(GEBcheckbox[i].parentNode.innerText);
//		tdd.innerHTML='<a href="' + ahref + '" target="_blank" class="lixian">离线下载</a>';
//}
tdd.width="160";
trd.insertBefore(tdd,trd.getElementsByTagName("td")[1]);
	GEBcheckbox.item(i).onclick=function() {
		var ii=0;
		for(i=0; i<GEBcheckbox.length; i++) {
			if(GEBcheckbox.item(i).checked==true){
				ii+=1;
			}
		}
		if (GEBcheckbox.length==ii){
			document.getElementsByName('qx').item(0).checked= true;
		}else{
			document.getElementsByName('qx').item(0).checked= false;
		}
	}
}
}
...
function kPlay(i){
	...
	try {
		var TempJq=jQuery(".sebc2 a.pClass").eq(i);
		var ver = dapCtrl.GetThunderVer("KANKAN", "INSTALL");
		var type = dapCtrl.Get("IXMPPACKAGETYPE");
		if(ver && type && ver >= 672 && type >= 2401)
		{
			var url=TempJq.attr("thunderHref");
			var moviename=TempJq.attr("thunderResTitle");
			...
			dapCtrl.Put("SXMP4ARG", '"'+url+'" /title "'+moviename+'" /startfrom "_web_ffdy" /openfrom "_web_ffdy"');	
		}
		else {
			var r = confirm("请先更新迅雷看看播放器或使用兼容模式访问本站,然后刷新本页面！");
			if(r == true){
				window.location.href= wtd_ChangFolder(14103);	
			}
		}
...
poxiao.score = {
	...
	data:{},
	get:function(){
		jQuery.getJSON("/ajax/?type=rank&action=videoscore&id="+video.vid+"", function(ret){
		poxiao.score.data=ret;
		poxiao.score.format();
		});
	},
	send:function(s){
		jQuery.get("/ajax/?type=rank",{id:video.vid,action:'score',score:s},function(msg){if(parseInt(msg)==1){alert('评分已提交，非常感谢您的参与！');}else{alert('囧,-_-|||，您已经评过分了哦？明天再来吧……');}});
	},
	format:function(){
		var data = poxiao.score.data;
		var s = data.s;
		var Num=Math.round(Math.round(s)/2);
		jQuery("#pingfen").html(s.toFixed(1));
		jQuery("#pingfen2").html(s.toFixed(1));
		for(var i=0;i<Num;i++)
		{
			jQuery("#starlist li a").eq(i).addClass("is");
		}
		jQuery("#startips").html(poxiao.score.texts[Num-1]);
		jQuery("#star-num").css("width",""+(s*10)+"%").attr("title","热度："+(s*10)+"%");
		jQuery("#total").css("width",""+(s*10)+"%");
	},
	hasSend:function(){
		var voted = jQuery.cookie('voted');
		if(voted==null||voted=='')
			return false;
		else{
	...
	init:function(){
		this.get();
		var star_width = 26;
		//jQuery('#star_current_rating').css('width',0);
		jQuery('#starlist > li').click(function(){
			if(!poxiao.score.hasSend()){
				var i = jQuery(this).attr('i');
				poxiao.score.send(i);
				//jQuery('#star_current_rating').css({'width':star_width*i});
				//poxiao.score.data.s[i-1]++;
			}else{
				alert('囧,-_-|||，您已经评过分了哦？明天再来吧……');
			}
		});
		jQuery('#starlist > li').hover(function(){
										jQuery("#starlist li a").removeClass("on");
										var Num = jQuery(this).attr('i');
										for(var i=0;i<6;i++)
										{
											if(i<Num)
											{
												...
												jQuery("#starlist li a").eq(i).removeClass("is");
											}
											jQuery("#starlist li a").eq(i).attr("title",poxiao.score.texts[Num-i]);
										}
										jQuery("#startips").html(poxiao.score.texts[Num-1]);
										/*   jQuery('#star_tip').show();
										   jQuery('#star_tip_arrow').css('left',star_width*i-20 + 'px');
										   jQuery('#star_current_rating').html(i + '星，' + poxiao.score.texts[5-i]);
										   },function(){
											   jQuery('#star_tip').hide();
										*/
										}
		);
	}
